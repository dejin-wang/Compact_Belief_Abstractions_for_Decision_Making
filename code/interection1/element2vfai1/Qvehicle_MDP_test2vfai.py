...
def measurement_model(state):
    Fai = [0, 1, 2]  # G Y R
    d, v, fai, tfai = state.numpy()[0]
    nd = np.random.normal(0, 1)  # Reduce noise variance
    nv = np.random.normal(0, 0.1)
...
if __name__ == '__main__':
    ...
    for i in range(numb):
        ...
        for t in count():
            ...
            if state.numpy()[0][0] < 0:
                ...
                re_particles, re_weights = particle_filter(num_particles, measurement, re_particles, re_weights,action_index)
                estimated_state = get_estimated_state(re_weights, re_particles)
    ...
    import csv
    with open('DQN_MDP_reward_test2vfai.csv', 'w', newline='') as csvfile:
