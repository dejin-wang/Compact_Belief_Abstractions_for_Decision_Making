...
def particle_filter(num_particles, measurement, particles, weights, action_index_est):
    ...
    for i in range(l):
        ...
        likelihood1 = (0.1 / np.sqrt(2 * np.pi)) * np.exp(-0.5 * 0.1*0.1 * (d_m - d) ** 2)
        likelihood2 = (1/ np.sqrt(2 * np.pi)) * np.exp(-0.5 * 1 * 1 * (v_m - v) ** 2)
        if fai == fai_m:
            likelihood3 = 0.91
        else:
            likelihood3 = 0.09
...
import csv
with open('DQN_particles_reward_test_high.csv', 'w', newline='') as csvfile:
