...
def get_standard_deviation(term):
    if term == 'min':
        dev= 1
        dev1=0.1
        p_w= 0.94
...
def measurement_model(state,term):
    dev,dev1,p_w = get_standard_deviation(term)
    Fai = [0, 1, 2]  # G Y R
    d, v, fai, tfai = state.numpy()[0]
    nd = np.random.normal(0, dev)
    nv = np.random.normal(0, dev1)
...
def compute_entropy(weights):
    ...
    entropy = -np.sum(weights * np.log(weights + 1e-300))  # 加上1e-10以避免log(0)

    return entropy

def particle_filter(num_particles, measurement, particles, weights, action_index_est,term):
    dev,dev1, p_w = get_standard_deviation(term)
    l = len(particles)
    #print(particles)
    ...
    for i in range(l):
        ...
        likelihood1 = ((1/dev) / np.sqrt(2 * np.pi)) * np.exp(-0.5 * (1/dev/dev) * (d_m - d) ** 2)
        likelihood2 = ((1/dev1)  / np.sqrt(2 * np.pi)) * np.exp(-0.5 * (1/dev1/dev1) * (v_m - v) ** 2)
        if fai == fai_m:
            likelihood3 = p_w+(1-p_w)/3
        else:
            likelihood3 = 2*(1-p_w)/3
        likelihood4 = 1
    ...
    return re_particles_t, re_weights_t,entropy
...
for i in range(numb):
    ...
    state = torch.tensor(state, dtype=torch.float32, device=device).unsqueeze(0)
    #print(state)
    measurement = measurement_model(state,term)
    ...
    for t in count():
        ...
        episode_entropy_set.append(episode_entropy)
        action_index = policy_net(estimated_state).max(1).indices.view(1, 1)
        done = (state.numpy()[0][0] < 0) or (t > 50)
        ...
        measurement = measurement_model(state,term)
        re_particles, re_weights,entropy = particle_filter(num_particles, measurement, re_particles, re_weights, action_index_est,term)
        print(entropy)
        #print(re_particles)
        ...
        episode_entropy = (episode_entropy*(t)+entropy)/(t+1)
    ...
    entropy_set.append(episode_entropy_set)
    print(f"Episode {i + 1}/{numb}, Reward: {episode_reward}")

longest_array = max(entropy_set, key=len)

for i in range(numb):
    if len(entropy_set[i]) < max(len(longest_array), 30):
        l = max(len(longest_array), 30) - len(entropy_set[i])
        for j in range(l):
            entropy_set[i].append(entropy_set[i][-1])

import csv
if term == 'min':
    name = 'DQN_particle_min_entropy_test.csv'
elif term == 'mid':
    name = 'DQN_particle_mid_entropy_test.csv'
else:
    name = 'DQN_particle_max_entropy_test.csv'

with open(name, 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    for episode_index, episode in enumerate(entropy_set):
