...
# 定义粒子滤波函数
def particle_filter(num_particles, measurement, particles, weights, action_index_est, t, threshold=0.5):
    l = len(particles)
    if is_multiple(ts, t):
        for i in range(l):
...
if __name__ == '__main__':
    ...
    for i_episode in range(num_episodes):
        ...
        for t in range(20):
            ...
            measurement = measurement_model(state)
            re_particles, re_weights = particle_filter(num_particles, measurement, re_particles, re_weights, action_index_est,t)
            estimated_state = get_estimated_state(re_weights, re_particles)
...
print(reward_set[0])
import csv
with open('DQN_reward_particle_testt5.csv', 'w', newline='') as csvfile:
